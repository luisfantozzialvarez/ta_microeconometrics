...
Let $Y_0$ and $Y_1$ denote potential ouctomes of a binary treatment $D$ in a population of interest, which is assigned according to a deterministic rule $D = 1\{T\geq 0 \}$, where $T$ is a __continuous running variable__. The observed outcome is $Y = D Y_1 + (1-D) Y_0$. Let  $\mu_0(t)=  \mathbb{E}[Y_0|T =t ]$, $\mu_{1}(t) = \mathbb{E}[Y_1|T=t]$, and $\mu(t) := \mathbb{E}[Y|T = t]$, where expectations are taken wrt the population distribution. You have seen in class that, under the continuity assumption that $\mu_0$ and $\mu_1$ are continuous at $t=0$ (what does this assumption mean?), the conditional average treatment effect at the cutoff, $\tau : = \mathbb{E}[Y_1 = Y_0|T=0]$ is __identified__ as:
...
where $K$ is a __kernel function__ and $h > 0$ is a __bandwidth__ parameter, to be defined by the user. The kernel measures the "distance" -- rescaled by h -- between the realizations of the $X_i$ and the point of interest $x$. Typical choices of kernel include: (1) the uniform kernel $K(s) = 1\{ -\frac{1}{2} \leq  s \leq \frac{1}{2}\}$; (2) the triangular kernel $K(s) = 1\{|s|\leq 1\} (1 - |s|)$; (3) the Epanechnikov kernel $K(s) = 1\{|s| \leq 1\} \frac{3}{4} (1- |s|^2) $; and the (4) Gaussian kernel $K(s) = \frac{1}{\sqrt{\pi}} \exp\left(-\frac{1}{2}s^2\right)$. The first three are the most common in the RD design, as they effectively discard observations far away from the point of interest. Let us plot these below:


```r
uniform <- function(s){dunif(s, min = -1/2,max = 1/2)}
...
#Plot
plot(grid_s, uniform(grid_s), ylab = "density", xlab = "value", type = "l")
...
$$
\hat{\mu}_x \in \operatorname{argmin}_{a \in \mathbb{R}} \sum_{i=1}^n K\left(\frac{X_i - x}{h}\right) (Z_i - a)^2 \quad (*)
$$
...
$$(\hat{b}_0, \hat{b}_1, \ldots \hat{b}_k) \in \operatorname{argmin}_{b_0, b_1 \ldots b_k} \sum_{i=1}^n K\left(\frac{X_i - x}{h}\right) \left(Z_i - \sum_{j=1}^k b_j \frac{(Z_i -x)^j}{j!}\right)^2$$
and the other $\hat{b}_j$ are estimates of derivatives o $\mu$ at $x$.
...
and estimates the treatment effect by the difference $\hat{\tau}(h^+, h_-) = \hat{\mu}^+ - \hat{\mu}^-$. The bandwidth can be chosen differently at the right and the left, though it is usually constrained to be equal. We assume this throughout ($h^+ = h^- = h_n$ ; the $n$ subscript is to emphasise that the bandiwdth is a function of the sample size) and write $\hat{\tau}(h_n)$ for the estimator.
...
$$\sqrt{nh} \frac{\left(\hat{\tau}(h_n) - \tau\right)}{\mathbb{V}[\hat{\tau}(h_n)|\mathbf{T}]^{\frac{1}{2}}} \to N(0,1$$
where $\mathbf{T} =(T_1,T_2\ldots T_n)$. This approximation could in principle be used as a basis for an inferential procedure on the treatment effect -- we note in particular that $\mathbb{V}[\hat{\tau}(h_n)|\mathbf{T}]$ could be estimated from the standard error output of a weighted regression using the kernel weights. The problem is that the leading approaches to choose $h_n$ -- which, as we discussed, work by minimizing approximations of the mean squared error -- lead to bandwidth choices (as a function of sample size) "too large" to satisfy $nh^5_n \to 0$, For example, the approach in  Imbens and Kalyanaraman (2012), which consists of minimising an asymptotic expansion of:

$$\mathbb{E}[(\hat{\tau}(h) - \tau)^2] = \text{Bias}(\hat{\tau}(h)) + \mathbb{V}[\hat{\tau}(h)]$$
leads to a choice of bandwidth $h_n = C n^{-1/5} $ where $C > 0$ is a constant, implying that $nh^5_n \nrightarrow 0$. In this case, it can be shown that the RDD estimator __is still consistent__, but that now it satisfies the asymptotic distribution:

$$\sqrt{nh} \frac{\left(\hat{\tau}(h_n) - B(h_n) -  \tau\right)}{\mathbb{V}[\hat{\tau}(h_n)|\mathbf{T}]^{\frac{1}{2}}} \to N(0,1)$$
...
where $f_T$ is the density of $T$; against an alternative that the densities are different. In an influential paper, McCrary (2008) originally suggested testing this null by estimating the density at either side of the cutoff using local linear regression on an histogram estimator of the cdf -- and then performing a t-test on the difference. This approach involved, along with the bandwidth selection of the local linear regression, selecting the binpoints for estimation of the histogram. More recently, Cattaneo et al. (2020) suggested estimating the density at either side by the coefficient $b_1$ of the local polynomial regression:
